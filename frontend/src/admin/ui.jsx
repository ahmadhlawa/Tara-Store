...
export const card = sx`background:#fff;border:1px solid #E9DED0;border-radius:14px;padding:18px`;
export const label = sx`display:flex;flex-direction:column;gap:6px;font-size:13px;font-weight:700;color:#4B3F41`;
export const input = sx`height:44px;border:1px solid #E2D5C6;border-radius:10px;background:#FDF8F2;padding:0 12px;font-size:14px;font-weight:400;font-family:inherit;width:100%;box-sizing:border-box`;
export const textarea = sx`border:1px solid #E2D5C6;border-radius:10px;background:#FDF8F2;padding:10px 12px;font-size:14px;font-weight:400;font-family:inherit;width:100%;box-sizing:border-box;resize:vertical`;

export function Button({ variant = "primary", children, style, ...rest }) {
  const variants = {
    primary: "background:#7D595B;color:#fff;border:1px solid #7D595B",
    secondary: "background:#fff;color:#7D595B;border:1px solid #7D595B",
    ghost: "background:#fff;color:#5C4D4F;border:1px solid #E2D5C6",
  ...
  return (
    <div style={sx`display:flex;align-items:flex-end;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:18px`}>
      <div style={sx`display:flex;flex-direction:column;gap:4px`}>
        <h1 style={sx`margin:0;font-size:22px;font-weight:800;color:#4B3F41`}>{title}</h1>
...
export function Notice({ kind = "info", children }) {
  ...
  const styles = {
    info: "background:#F5EDEE;border-color:#E2D2D4;color:#7D595B",
    ...
    <div style={sx`overflow-x:auto`}>
      <table style={sx`width:max-content;min-width:100%;border-collapse:collapse;font-size:13.5px`}>
        <thead>
          <tr>
            {columns.map((column) => (
              <th key={column.key} style={sx`text-align:start;padding:10px 12px;border-bottom:1px solid #E9DED0;color:#766669;font-weight:700;white-space:nowrap`}>
                {column.title}
              </th>
            ))}
          </tr>
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={title} style={sx`position:fixed;inset:0;z-index:120;background:rgba(26,24,21,.45);display:flex;align-items:center;justify-content:center;padding:16px`}>
      <div style={sx`background:#fff;border-radius:16px;width:min(${wide ? "900px" : "560px"},100%);max-height:90vh;overflow-y:auto;box-shadow:0 30px 70px rgba(26,24,21,.3)`}>
        <div style={sx`display:flex;align-items:center;justify-content:space-between;gap:12px;padding:16px 20px;border-bottom:1px solid #E9DED0;position:sticky;top:0;background:#fff`}>
          <strong style={sx`font-size:16px`}>{title}</strong>
          <button type="button" onClick={onClose} aria-label="إغلاق" style={sx`width:36px;height:36px;border:1px solid #E9DED0;border-radius:9px;background:#fff;cursor:pointer;font-size:15px`}>✕</button>
        </div>
        <div style={sx`padding:20px;display:flex;flex-direction:column;gap:14px`}>{children}</div>
        {footer && <div style={sx`padding:14px 20px;border-top:1px solid #E9DED0;display:flex;gap:10px;justify-content:flex-end;position:sticky;bottom:0;background:#fff`}>{footer}</div>}
      </div>
    </div>
  );
}
...
export function ConfirmDialog({ title, message, confirmLabel = "تأكيد الحذف", onConfirm, onCancel }) {
  return (
    ...
    >
      <p style={sx`margin:0;font-size:14px;line-height:1.9;color:#5C4D4F`}>{message}</p>
    </Modal>
  );
}
...
export function Badge({ tone = "neutral", children }) {
  const tones = {
    neutral: "background:#F5EDE3;color:#5C4D4F",
