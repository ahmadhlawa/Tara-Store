...
export { CategoryPicker } from "../CategoryPicker.jsx";
    ...
    <>
      ...
      <Section title="البيانات الأساسية">
        <div style={sx`display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px`}>
          ...
          <div style={label}>القسم<CategoryPicker categories={categories} value={form.category_id} onChange={(categoryId) => update({ category_id: categoryId })} /></div>
              ...
              <div key={`presentation-${value.id ?? valueIndex}`} style={sx`display:flex;flex-direction:column;gap:8px;padding:10px;border:1px solid #E7DCF2;border-radius:10px`}>
                ...
                <div style={sx`display:flex;gap:8px;flex-wrap:wrap`}>
                  {(value.images || []).map((image, imageIndex) => (
                    <div key={`${image.url}-${imageIndex}`} style={sx`width:100px;min-width:0;display:flex;flex-direction:column;gap:5px`}>
                      <img src={image.url} alt="" style={sx`display:block;width:100%;height:100px;object-fit:cover;border-radius:8px`} />
                      <div style={sx`width:100%;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:3px`}>
                        <Button variant="danger" aria-label={`حذف الصورة ${imageIndex + 1}`} onClick={() => setOptions((rows) => rows.map((row, i) => i === optionIndex ? { ...row, values: row.values.map((item, j) => j === valueIndex ? { ...item, images: item.images.filter((_, k) => k !== imageIndex) } : item) } : row))} style={sx`width:100%;min-width:0;min-height:32px;padding:0`}>×</Button>
                        <Button variant="ghost" aria-label={`تحريك الصورة ${imageIndex + 1} للسابق`} disabled={imageIndex === 0} onClick={() => setOptions((rows) => rows.map((row, i) => i === optionIndex ? { ...row, values: row.values.map((item, j) => j === valueIndex ? { ...item, images: item.images.map((entry, k, all) => k === imageIndex - 1 ? all[imageIndex] : k === imageIndex ? all[imageIndex - 1] : entry) } : item) } : row))} style={sx`width:100%;min-width:0;min-height:32px;padding:0`}>↑</Button>
                        <Button variant="ghost" aria-label={`تحريك الصورة ${imageIndex + 1} للتالي`} disabled={imageIndex === value.images.length - 1} onClick={() => setOptions((rows) => rows.map((row, i) => i === optionIndex ? { ...row, values: row.values.map((item, j) => j === valueIndex ? { ...item, images: item.images.map((entry, k, all) => k === imageIndex + 1 ? all[imageIndex] : k === imageIndex ? all[imageIndex + 1] : entry) } : item) } : row))} style={sx`width:100%;min-width:0;min-height:32px;padding:0`}>↓</Button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {!isNew && (
        <>
          ...
          <Section collapsible title="حقول متقدمة وتسويقية">
            ...
            <div style={sx`display:flex;gap:18px;flex-wrap:wrap`}>
              {[["show_on_home", "عرض في الصفحة الرئيسية"], ["is_featured", "مميّز"], ["is_new", "جديد"], ["is_bestseller", "الأكثر مبيعاً"]].map(([key, title]) => (
                <label key={key} style={sx`display:flex;align-items:center;gap:8px;font-size:14px;font-weight:600;cursor:pointer`}>
