...
export function InvoiceDetailPage() {
  ...
  return <>
    ...
    <section aria-label="تفاصيل الفاتورة" style={{ ...card, ...sx`margin-bottom:14px` }}><div style={sx`display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px`}><Badge tone={tone(invoice.status)}>{invoiceStatusLabels[invoice.status] || invoice.status}</Badge><Badge tone={tone(invoice.payment_status)}>{paymentStatusLabels[invoice.payment_status] || invoice.payment_status}</Badge><span style={sx`font-size:13px;color:#766669`}>{paymentMethodLabels[invoice.payment_method] || invoice.payment_method}</span></div>
      ...
      <dl aria-label="ملخص الأسعار النهائية" style={sx`margin:16px 0 0;display:grid;grid-template-columns:1fr auto;gap:8px;max-width:360px;margin-inline-start:auto`}><dt>المجموع الفرعي</dt><dd>{amount(invoice.subtotal, symbol)}</dd><dt>التوصيل</dt><dd>{amount(invoice.delivery_fee, symbol)}</dd><dt style={sx`font-weight:800;border-top:1px solid #7F568F;padding-top:8px`}>الإجمالي النهائي</dt><dd style={sx`font-weight:800;border-top:1px solid #7F568F;padding-top:8px`}>{amount(invoice.grand_total, symbol)}</dd><dt>المدفوع / المسترد / المتبقي</dt><dd>{amount(invoice.paid_amount, symbol)} / {amount(invoice.refunded_amount, symbol)} / {amount(invoice.remaining_amount, symbol)}</dd></dl>
    </section>
