...
export default function ContactRoutePage() {
  ...
  useSeo({ title: t("تواصل معنا | {0}", [settings.storeName]), description: settings.seoDescription || settings.tagline, baseUrl: settings.publicBaseUrl, path: "/contact" });
  useEffect(() => {
    let cancelled = false;
  ...
  const methods = [
    settings.whatsapp && { type: "whatsapp", label: t("واتساب"), value: settings.whatsapp, href: whatsappHref(settings.whatsapp, "") },
    settings.instagramVisible && settings.instagram && { type: "instagram", label: t("إنستغرام"), value: instagramValue(settings.instagram), href: settings.instagram },
  ].filter(Boolean);
